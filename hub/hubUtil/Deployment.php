...
namespace hub\hubUtil;

use AlterTable;
use database\sqlCreator\Create;
use database\sqlCreator\databaseElement\Column;
use database\sqlCreator\databaseElement\ColumnType;
use hub\hubUtil\dbStructure\TableCreator;
...
class Deployment {
    ...
    private static function getStructure(){
        $path = __DIR__.DIRECTORY_SEPARATOR.'dbStructure';
        $files = \Hub::get()->File->getFilesInDir($path);
        $result = [];
        foreach ($files as $file) {
            if($file['filename'] == 'TableCreator'){continue;}
            $cName = 'hub\\hubUtil\\dbStructure\\'.$file['filename'];
    ...
    private static function importDefaults($name, $values, $columns){
        ...
        $result = \Hub::get()->Db->query($sql->parse());
        if($result){
            ...
            self::log(self::LEVEL_INFO, "[SQL CODE] ".\Hub::get()->Db->getContext()->error);
    ...
    private static function createTable($name, $data){
        ...
        if($result){
            ...
            self::log(self::LEVEL_FATAL, '[MYSQL] '.\Hub::get()->Db->getContext()->error);
    ...
    private static function tableExists($name){
        ...
        $result = \Hub::get()->Db->query($query->parse());
        return ($result !== null);
    }
    ...
    private static function tableStructureCheck($name, $data){
        $query = (new \Select('COLUMN_NAME'))
            ...
            ->where([
                'TABLE_SCHEMA' => \Constants::getDbName(),
                'TABLE_NAME' => $name
            ]);
        $result = \Hub::get()->Db->query($query->parse());
        ...
        if(count($missing) > 0){
            ...
            if(\Hub::get()->Db->query($alter->parse())){
